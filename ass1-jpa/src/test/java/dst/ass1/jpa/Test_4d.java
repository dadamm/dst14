...
public class Test_4d extends AbstractTest {
	...
	public void testInterceptor() {
		...
		Query c = em.createQuery("select c from " + Constants.T_EXPERT + " c");
		c.getResultList();
		
		assertEquals(1, SQLInterceptor.getSelectCount());
		
		c = em.createQuery("select distinct c from " + Constants.T_EXPERT + " c");
		c.getResultList();
		
		assertEquals(2, SQLInterceptor.getSelectCount());
		
		c = em.createQuery("select e from " + Constants.T_TASKFORCE + " e");
		c.getResultList();
