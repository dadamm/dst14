...
public final class EventingUtils {
	...
	/**
	 * Checks if {@code taskId} from bean matches ANY in {@code compareWith}.
	 ...
	 *            the event bean
	 * @param compareWith
	 *            the list of possible taskIds
	 */
	public static void ensureTaskId(String failMsg, EventBean e,
			long... compareWith) {
		StringBuffer couldBe = new StringBuffer();
		try {

			Long id = getLong(e, "taskId");
			...
			fail(String.format("%s : taskId should be in ( %s) but was %d",
					failMsg, couldBe, id));
		} catch (Exception b) {
			b.printStackTrace();
		}
